...
class Parser
{
	...
	protected function parse_readme($file_or_url)
	{
		$context = stream_context_create([
			'http' => ['user_agent' => 'WordPress.org Plugin Readme Parser'],
		]);
	...
	protected function parse_readme_contents($contents)
	{
		...
		if (strtolower($this->name) === 'plugin name') {
			$this->warnings['invalid_plugin_name_header'] = true;
	...
	{
		if (strpos($line, ':') === false || strncmp($line, '#', strlen('#')) === 0 || strncmp($line, '=', strlen('=')) === 0) {
			return false;
		}
	...
	protected function parse_section($lines)
	{
		...
		for ($i = 0; $i < $line_count; $i++) {
			...
			$is_heading = false;
			if ($heading_style === 'heading' && ( $trimmed[0] === '#' || $trimmed[0] === '=' )) {
				$is_heading = true;
			} elseif ($heading_style === 'bold' && ( substr($trimmed, 0, 2) === '**' && substr($trimmed, -2) === '**' )) {
				$is_heading = true;
			}
	...
	public function validate_license($license)
	{
		...
		$sanitize_license = static function ($license) {
			...
			if (stripos($license, 'GNU GENERAL PUBLIC LICENSE Version 2, June 1991 Copyright (C) 1989') === 0) {
				$license = 'gplv2';
			}
